...
- Extendly를 빠르고 깔끔한 개발자용 익스텐션 마켓플레이스처럼 다듬는다.
...
- 메인/검색은 Raycast Store처럼 command/search 중심으로 만든다.
- 카드는 Raycast Store와 Vercel Marketplace처럼 작고 정보 밀도 있게 만든다.
...
- AI 큐레이터는 챗봇보다 command input과 추천 결과 중심으로 만든다.
...
7. card grid를 데스크톱 3열 또는 4열, 태블릿 2열, 모바일 1열로 만든다.
8. `ExtensionCard`, `ExtensionListItem`, `RecommendationCard` variant를 계획한다.
9. 상세 페이지 2-column 레이아웃이 모바일에서 1-column으로 접히는지 확인한다.
10. 등록 폼의 비활성, 로딩, 완료, 실패 버튼 상태를 분리한다.
11. AI 큐레이터 progress panel에는 내부 추론이 아니라 `RAG 검색`, `MCP 조회`, `추천 조합 생성` 같은 안전한 단계만 표시한다.
12. 중복되는 UI 상태를 재사용 컴포넌트로 분리할지 판단한다.
...
- 힌트 4: UI 지시서의 warm neutral 토큰을 기본으로 쓰되, accent는 RAG match나 AI 실행 상태처럼 의미가 있을 때만 쓴다.
- 힌트 5: 큰 히어로보다 검색창, 결과 수, 필터, 카드의 스캔 가능성이 더 중요하다.
